...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 lg:p-8">
        <form method="POST" action="<?= $action ?>" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label for="imagem" class="block text-sm font-medium text-gray-700 mb-1">Imagem
                    <?= $produto ? '(deixe em branco para manter)' : '*' ?></label>
                <?php if ($produto && isset($produto['imagem'])): ?>
                    <div class="mb-2">
                        <img src="<?= htmlspecialchars(imageUrl($produto['imagem'])) ?>" alt="Imagem atual"
                            class="h-32 w-32 object-cover rounded">
                    </div>
                <?php endif; ?>
                <input type="file" id="imagem" name="imagem" <?= $produto ? '' : 'required' ?>
            ...
            <div>
                <label for="tipoInstalacao" class="block text-sm font-medium text-gray-700 mb-1">Tipo de
                    Instalação</label>
                <input type="text" id="tipoInstalacao" name="tipoInstalacao"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    <label for="preco2" class="block text-sm font-medium text-gray-700 mb-1">Preço 2 (6 a 15 dias)
                        *</label>
                    <input type="number" id="preco2" name="preco2" step="0.01" min="0" required
                        value="<?= htmlspecialchars($produto['preco2'] ?? $produto['precoLongo'] ?? $data['preco2'] ?? $data['precoLongo'] ?? '') ?>"
                        class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary">
                </div>

                <div>
                    <label for="preco3" class="block text-sm font-medium text-gray-700 mb-1">Preço 3 (16 a 30 dias)
                        *</label>
                    <input type="number" id="preco3" name="preco3" step="0.01" min="0" required
            ...
            <div>
                ...
                <textarea id="descricao" name="descricao" rows="4" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary"><?= htmlspecialchars($produto['descricao'] ?? $data['descricao'] ?? '') ?></textarea>
            </div>
