...
<div class="max-w-8xl mx-auto">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <a href="/admin/produtos/create"
            class="bg-primary text-white py-2 px-4 rounded-md hover:bg-opacity-90 transition-colors font-medium">
            + Novo Produto
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 lg:hidden gap-4" id="produtos-mobile-list">
        <?php foreach ($produtos as $produto): ?>
            <div class="bg-white rounded-lg shadow-md p-4 produto-item" data-id="<?= $produto['id'] ?>">
                <img src="<?= htmlspecialchars(imageUrl($produto['imagem'])) ?>"
                ...
                <p class="text-sm text-gray-600 mb-4">Preço 3 (16-30 dias): R$
                    <?= number_format($produto['preco3'] ?? $produto['precoLongo'] ?? 0, 2, ',', '.') ?></p>
                <div class="flex gap-2">
                    <a href="/admin/produtos/<?= $produto['id'] ?>/edit"
                        class="flex-1 bg-blue-500 text-white py-2 px-4 rounded text-center text-sm hover:bg-blue-600">
                        Editar
                    </a>
                    <form method="POST" action="/admin/produtos/<?= $produto['id'] ?>/delete" class="flex-1"
                        onsubmit="return confirm('Tem certeza que deseja excluir este produto?');">
                        <button type="submit"
                            class="w-full bg-red-500 text-white py-2 px-4 rounded text-sm hover:bg-red-600">
                            Excluir
                        </button>
                    </form>
                </div>
        ...
        <table class="w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Imagem
                    </th>
                ...
                <?php foreach ($produtos as $produto): ?>
                    <tr class="produto-row cursor-move hover:bg-gray-50 transition-colors" data-id="<?= $produto['id'] ?>">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <img src="<?= htmlspecialchars(imageUrl($produto['imagem'])) ?>"
                                alt="<?= htmlspecialchars($produto['nome']) ?>" class="h-16 w-16 object-cover rounded">
                        </td>
                        <td class="px-6 py-4" style="max-width: 300px;">
                            <div class="text-sm font-medium text-gray-900 break-words">
                                <?= htmlspecialchars($produto['nome']) ?></div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">R$
                                <?= number_format($produto['preco1'] ?? $produto['precoCurto'] ?? 0, 2, ',', '.') ?></div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">R$
                                <?= number_format($produto['preco2'] ?? $produto['precoLongo'] ?? 0, 2, ',', '.') ?></div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">R$
                                <?= number_format($produto['preco3'] ?? $produto['precoLongo'] ?? 0, 2, ',', '.') ?></div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="/admin/produtos/<?= $produto['id'] ?>/edit"
                                class="text-blue-600 hover:text-blue-900 mr-4">Editar</a>
                            <form method="POST" action="/admin/produtos/<?= $produto['id'] ?>/delete" class="inline"
                                onsubmit="return confirm('Tem certeza que deseja excluir este produto?');">
    ...
    <?php if (empty($produtos)): ?>
        <div class="bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <a href="/admin/produtos/create" class="mt-4 inline-block text-primary hover:underline">Criar primeiro
                produto</a>
        </div>
    <?php endif; ?>
</div>
